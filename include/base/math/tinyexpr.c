...
static te_expr *new_expr(int const type, te_expr const *parameters[])
{
	...
	int const size = (sizeof(te_expr) - sizeof(void *)) + psize + (IS_CLOSURE(type) ? sizeof(void *) : 0);
	...
	do
	{
		...
		else
		{
			...
			if (isalpha(s->next[0]))
			{
				...
				te_variable const *var = find_lookup(s, start, s->next - start);
				if (!var)
